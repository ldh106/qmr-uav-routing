...
class GeoRouting(BASE_routing):
    ...
    def feedback(self, reward, src_id, best_action):
        """
        GeoRouting은 학습 기반(Q-learning)이 아니므로 feedback을 사용하지 않는다.
        BASE_routing에서 공통적으로 feedback()을 호출하는 경우를 대비한 no-op.
        """
        return
    def relay_selection(self, opt_neighbors, packet):
        ...
        for item in opt_neighbors:
            ...
            if isinstance(item, tuple) and len(item) == 2:
                hello_pkt, neighbor = item
            else:
               
                neighbor = item
                hello_pkt = self.hello_messages.get(neighbor.identifier, None)
                if hello_pkt is None:
                    continue  
