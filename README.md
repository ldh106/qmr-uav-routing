# 🛰️ QMR: Q-learning 기반 UAV 애드혹 네트워크 라우팅

> 졸업논문/캡스톤 프로젝트 (3인 팀)

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Simulation](https://img.shields.io/badge/Domain-UAV_Networking-4285F4?style=flat-square)
![Q-Learning](https://img.shields.io/badge/Q--Learning-Reinforcement_Learning-FF6F00?style=flat-square)
...
**팀이 이 위에 직접 추가한 것**
- 🔧 `alg_suffix` 기반 파라미터 주입 프레임워크
- 🧪 실험 시나리오 설계
- ⏱️ **Deadline-Aware Two-Mode Policy** 확장
...
| 🔧 해결방안 | 단순 파라미터 조정만으로는 개선이 어렵다고 판단, 의사결정 구조 자체를 바꾸는 실험 2로 이어짐 |

<img src="results/corrected_s3_delivery_ratio.png" width="450">

## 🧪 실험 2. Deadline-Aware Two-Mode Policy

| | 내용 |
|---|---|
| 💭 예상 | 마감(TTL)이 임박한 패킷만 목적지 방향으로 강제 수렴시키면 최악의 지연 사례가 줄어들 것 |
| 📊 실제 결과 | 최악 지연(max)·상위 5% 지연(p95) 모두 일관되게 감소 (예: max 584s → 578s). 다만 신뢰구간이 겹쳐 통계적으로는 조심스러움 |
| 🔧 해결방안 | seed 수를 늘려 추가 검증 필요. 페널티(DM)를 같이 켰을 때 효과가 안 보이는 문제는 원인을 찾아 별도 기록 → [NOTES.md](NOTES.md) 참고 |
...
Liu, J. et al. (2020). QMR: Q-learning based Multi-objective optimization Routing
protocol for Flying Ad Hoc Networks. *Computer Communications*, 150, 304–316.
(저작권 문제로 PDF는 첨부하지 않으며, 위 DOI 링크로 원문을 확인할 수 있습니다.)
